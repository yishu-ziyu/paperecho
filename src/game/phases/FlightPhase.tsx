...
export function FlightPhase() {
  ...
  return (
    <div className="relative flex flex-1 flex-col items-center justify-between overflow-hidden px-4 py-8">
      <Guide
        tone="night"
        title={searching ? (echo ? `${echo.name} 在读你的信` : "飞机在找一个相似的人") : note || "到了"}
        body={
          searching
            ? "这不是检索。是另一个人把你折进去的句子摊开。"
            : echo
              ? `${echo.city}。把飞机往下拉，落到桌上。`
              : "夜里那颗亮点，是另一个也把句子写下来的人。"
        }
      />
      ...
      <div className="relative flex flex-1 items-center justify-center">
        ...
        {found ? (
          ...
          <motion.div
            className="pointer-events-none relative z-[1] h-14 w-28 will-change-transform"
