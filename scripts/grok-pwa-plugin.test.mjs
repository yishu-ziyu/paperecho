...
test("never injects the Grok App Builder extensions script", () => {
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  // 隔离 cwd：不读项目真实的 site.json / public/og.jpg，测模板通用行为
  const out = injectGrokPwaHead(html, {
    appName: "Wild Race",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-chrome-")),
  });
...
test("emits og:image for a public host and prefers a custom card", () => {
  ...
  const customRoot = mkdtempSync(join(tmpdir(), "grok-og-custom-"));
  mkdirSync(join(customRoot, "public"));
  writeFileSync(join(customRoot, "public/og.jpg"), "x");
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  ...
  const invalid = injectGrokPwaHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-badcolor-")),
  ...
  const custom = injectGrokPwaHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-nocolor-")),
...
test("document title entities are not double-escaped on og:title", () => {
  const out = injectGrokPwaHead(
    ...
    { cwd: mkdtempSync(join(tmpdir(), "grok-og-entities-")) },
  );
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", {
    appName: "Solo",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-nohead-")),
  });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({
    appName: "Wild Race",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-streaming-")),
  });
  const chunks = [
...
test("strips a leftover Grok App Builder extensions script", () => {
  const ctx = { appName: "Demo", projectId: "proj-123" };
  const dirty =
    '<html><head><script src="https://grok.com/grok-app-builder/extensions.js" defer></script></head></html>';
  const once = injectGrokPwaHead(dirty, ctx);
  const twice = injectGrokPwaHead(once, ctx);
  assert.equal(once, twice);
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    appName: "Wild Race",
    cwd: mkdtempSync(join(tmpdir(), "grok-og-appname-")),
  });
