...
> 对齐当前飞行到达口径（2026-08-23）：找到人后不再演「穿过来了」窗光穿梭，直接用 `searchNote` 下拉落地。  
...
| | |
|---|---|
| **玩家看见** | 一张纸卡。`PAPER` / `ECHO` 进场错开弹出。副文「把靠近你的留下来，折成会回来的纸飞机。」有过局则底部多一个「信柜 N」。 |
...
| | |
|---|---|
| **玩家看见** | 飞机在云里飞。寻找中：「在夜里找一个也说过类似话的人」。找到后云向两边散，文案用 `searchNote`：「到了 {城}，{名} 读完了你的信」。没有窗光圆环，也不再说「穿过来了」。 |
| **手** | 寻找中跟飞。找到后下拉，过 48。最短飞行 5 秒。 |
